...
export default async function page() {
    ...
    let response=await fetch("https://api.themoviedb.org/3/movie/popular?api_key=7e2e58ffcb7347489a7974526d3318b9")
     ...
     <div className='grid grid-cols-2 cursor-pointer gap-3 mt-4 mb-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-4  '>
        {data.results.map((item:number|string,index:number) =>{
            ...
            return <div key={index}>
                    ...
                    <p className='text-black  dark:text-white text-center'>{data.results[index].original_title}</p>
                    ...
                    <div className='flex justify-between'>
                        ...
                        <div>
                        <p className='text-yellow-600'><i className="fa-solid fa-star"></i></p>
                        </div>
                    </div>
                    </div>
                    </Link>
